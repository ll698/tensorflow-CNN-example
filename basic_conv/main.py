...
training = [data_dict0,data_dict1, data_dict2, data_dict3]
testing = [data_dict_test]

trainX, trainY = return_matrix_form(training)

subset = np.random.randint(10000, size=6000)
...
"""
    ...
    in which we will assign data to at a layer point.
...
"""
    ...
    The negative one indidicates there can be a variable number for this dimension for our model.
"""
...
W_fc1 = weight_tensor([8*8*64, 64])
b_fc1 = bias_tensor([64])

h_pool2_flat = tf.reshape(h_pool2, [-1, 8*8*64])
h_fc1 = tf.nn.relu(tf.matmul(h_pool2_flat, W_fc1) + b_fc1)

#Second densely connected layer
W_fc2 = weight_tensor([64, 64])
b_fc2 = bias_tensor([64])

h_fc2 = tf.nn.relu(tf.matmul(h_fc1, W_fc2) + b_fc2)

#readout layer
W_fc3 = weight_tensor([64, 10])
...
"""
    ...
    as using gradient descent with out defined learning rate to minimize the cross entropy function
"""
train_step = tf.train.GradientDescentOptimizer(learning_rate).minimize(cross_entropy)

"""
    We define our definition of a correct prediction, our model outputs a 10 dimensional vector
    where each dimension represents the probability of the inputted image being that class,
...
"""
print('Training neural network on a subset of cifar10 containing 30,000 training samples and 6,000 testing samples...')
print('training iterations: 1000...')
for i in range(8000):

    #We pull a batch of samples from our training data to train on
    subset = np.random.randint(30000, size=128)
...
print("Testing accuracy: %i" %(test_accuracy))
